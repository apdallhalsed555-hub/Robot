...
4. [Orchestration Logic — The Brain Between STT and TTS](#4-orchestration-logic--the-brain-between-stt-and-tts)
5. [Web Search Tool](#5-web-search-tool)
6. [End-to-End Latency Analysis](#6-end-to-end-latency-analysis)
7. [Summary of Engineering Decisions](#7-summary-of-engineering-decisions)
...
graph LR
    subgraph "Main Thread"
        MIC["🎙️ Microphone"] --> STT["STT Engine<br/>(faster-whisper)"]
        STT --> |"SpeechEvent<br/>Priority 0"| BUS["Event Bus<br/>(PriorityQueue)"]
        BUS --> BRAIN["Orchestration Logic<br/>(LLM + Tools)"]
        BRAIN --> |"Token Stream"| TTS["TTS Engine<br/>(Kokoro)"]
        TTS --> SPK["🔊 Speaker"]
        BRAIN --> |"Tool Call"| WEB["Web Search<br/>(Tavily + DDG)"]
        WEB --> |"Results"| BRAIN
        SPK -.-> |"Barge-in<br/>Interrupt"| STT
    end

    style STT fill:#1e3a5f,stroke:#4a90d9,color:#fff
    style TTS fill:#1e3a5f,stroke:#4a90d9,color:#fff
    style BRAIN fill:#2d1b4e,stroke:#8b5cf6,color:#fff
    style WEB fill:#1b4332,stroke:#40c057,color:#fff
    style BUS fill:#4a3728,stroke:#e67e22,color:#fff
```
...
sequenceDiagram
    ...
    LLM->>SB: Token: " some"
    LLM->>SB: Token: " info."
    Note over SB: Sentence boundary detected (period)
    SB->>TTS: "I found some info."
    TTS->>SPK: 🔊 Play audio (while LLM continues)
    LLM->>SB: Token: " Would"
    LLM->>SB: Token: " you"
    LLM->>SB: Token: " like"
    LLM->>SB: Token: " more?"
    SB->>TTS: "Would you like more?"
    TTS->>SPK: 🔊 Play audio
```
...
```mermaid
graph TD
    subgraph "Thread 1: _listen_loop"
        A["PyAudio Stream<br/>16-bit Mono, 16kHz"] --> B["Raw Chunks<br/>1024 samples each"]
        B --> C["Audio Queue<br/>(thread-safe)"]
    end

    subgraph "Thread 2: _process_loop"
        C --> D["Convert to Float32<br/>Normalize -1.0 to 1.0"]
        D --> E{"Calculate RMS Volume"}
        E --> |"RMS >= 0.02"| F["Accumulate Speech"]
        E --> |"RMS < 0.02 for 0.8s"| G["Finalize Utterance"]
        F --> |"Duration > 30s"| H["Stitch: Transcribe Chunk"]
        G --> I["Transcribe + Filter"]
        I --> J["Push SpeechEvent<br/>to EventQueue"]
    end

    style A fill:#1e3a5f,stroke:#4a90d9,color:#fff
    style I fill:#2d1b4e,stroke:#8b5cf6,color:#fff
    style J fill:#1b4332,stroke:#40c057,color:#fff
```
...
```mermaid
graph TD
    A["Raw Whisper Output"] --> B{"Is Repetitive Garbage?"}
    B --> |"Yes (>= 45% same word)"| X["❌ Discard"]
    B --> |No| C{"Word Count < 3?"}
    C --> |Yes| D{"Is Legitimate Short Phrase?"}
    D --> |"'hello', 'yes', 'no', etc."| E["✅ Accept"]
    D --> |"'I am Ahmed', 'name is...'"| E
    D --> |Neither| X
    C --> |No| F{"Is Known Hallucination?"}
    F --> |"'thank you', 'subscribe', etc."| X
    F --> |No| G{"Alphabetic Ratio >= 30%?"}
    G --> |No| X
    G --> |Yes| H{"Avg LogProb >= -0.85?"}
    H --> |No| X
    H --> |Yes| E

    style X fill:#8b1a1a,stroke:#dc3545,color:#fff
    style E fill:#1b4332,stroke:#40c057,color:#fff
```
...
```mermaid
graph LR
    STT["STT Engine"] --> |"SpeechEvent<br/>Priority 0 (highest)"| PQ["PriorityQueue"]
    VIS["Vision Pipeline"] --> |"VisionChangeEvent<br/>Priority 1 (lower)"| PQ
    PQ --> |"Dequeue by priority<br/>then timestamp"| MAIN["Main Loop<br/>(main.py)"]

    style PQ fill:#4a3728,stroke:#e67e22,color:#fff
```
...
```mermaid
stateDiagram-v2
    [*] --> Listening: System Boot

    Listening --> AccumulatingSpeech: RMS >= SILENCE_THRESHOLD (0.02)
    AccumulatingSpeech --> Listening: Silence > 0.8s then Finalize

    Listening --> RobotSpeaking: TTS starts playback
    RobotSpeaking --> BargeIn: User RMS >= BARGE_IN_RMS (0.055)
    RobotSpeaking --> PostTTSCooldown: TTS playback ends

    BargeIn --> AccumulatingSpeech: TTS interrupted, buffer reset
    PostTTSCooldown --> Listening: 650ms elapsed
    PostTTSCooldown --> BargeIn: User RMS >= BARGE_IN_RMS
```
...
```mermaid
graph TD
    A["LLM Response"] --> B{"Has native tool_calls?"}
    B --> |Yes| C["Execute Tool Normally"]
    B --> |No| D{"Contains XML-style function tag?"}
    D --> |Yes| E["Parse function tag and args"]
    E --> C
    D --> |No| F["Return text response"]

    C --> G["Append ToolMessage to context"]
    G --> H["Call LLM again with tool result"]
    H --> B

    A --> |"400 Bad Request"| I{"tool_use_failed error?"}
    I --> |Yes| J["Immediately raise<br/>(no API key rotation)"]
    J --> K["Parse failed_generation field"]
    K --> L["Extract function name + args"]
    L --> C

    style J fill:#8b1a1a,stroke:#dc3545,color:#fff
    style C fill:#1b4332,stroke:#40c057,color:#fff
```
...
graph TD
    subgraph "Short-Term Memory (STM)"
        A["New Exchange"] --> B["Buffer: Last 5 exchanges<br/>(10 messages)"]
        B --> |"Buffer full"| C["LLM Summarization"]
        C --> D["Rolling Summary Paragraph"]
        D --> E["Keep last 2 exchanges<br/>+ summary"]
    end

    subgraph "Long-Term Memory (LTM)"
        F["save_to_memory tag"] --> G["BGE-small-en-v1.5<br/>384-dim embedding"]
        G --> H["Qdrant Vector Store<br/>per-user partitioned"]
    end

    subgraph "RAG Prefetch"
        I["User speaks"] --> J["Embed query"]
        J --> K["Search Qdrant<br/>top-3 results"]
...
```mermaid
graph TD
    A["Search Query"] --> B{"Contains Arabic?"}
    B --> |Yes| C["Translate via Groq LLM<br/>(temperature=0.0)"]
    C --> D["English Query"]
    B --> |No| D

    D --> E{"Tavily API Key Set?"}
    E --> |Yes| F["Tavily Search<br/>(search_depth=advanced)"]
    F --> |Success| G["Format and Return Results"]
    F --> |Failure| H["DuckDuckGo Text Search"]

    E --> |No| H
    H --> |Success| G
    H --> |No Results| I["DuckDuckGo News Search"]
    I --> |Success| G
    I --> |No Results| J["Return: No results found"]

    style F fill:#1b4332,stroke:#40c057,color:#fff
    style H fill:#1e3a5f,stroke:#4a90d9,color:#fff
    style I fill:#4a3728,stroke:#e67e22,color:#fff
```
...
gantt
    ...
    Audio Capture + VAD         :a1, 0, 350
    STT Transcription           :a2, after a1, 480
    RAG Prefetch                :a3, after a2, 45
    LLM Full Generation         :a4, after a3, 290
    TTS Full Synthesis          :a5, after a4, 400
    User hears first word       :milestone, after a5, 0

    section With SentenceBuffer (My Design)
    Audio Capture + VAD         :b1, 0, 350
    STT Transcription           :b2, after b1, 480
    RAG Prefetch                :b3, after b2, 45
    LLM First Sentence          :b4, after b3, 150
    TTS First Sentence          :b5, after b4, 120
    User hears first word       :milestone, after b5, 0
```
